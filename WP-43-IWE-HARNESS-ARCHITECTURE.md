---
type: composite-decision-support-carrier
wp: WP-43
...
| Поле | Значение |
|---|---|
| **Текущий вопрос** | Следует ли принять вариант E — «локальный каркас практики + выборочное использование источников + Control Center + сначала надёжный одиночный запуск, затем ограниченный цикл» — как текущее направление архитектуры личной IWE? |
| **`EntityOfConcern`** | `ArchitectureDecisionRelation@Project` с идентификатором `PAD-IWE-001` |
| **Применённый `Solution`** | `C.32.PAD — Project Architecture Decision After Candidate Synthesis` |
| **Точный вид результата** | `ArchitectureDecisionRelation@Project` |
| **Состояние** | `proposedForHumanDecision`: отношение заполнено как предложение, но решение владельца ещё не записано |
| **Получатель следующего хода** | `amorales`, владелец архитектуры личной IWE и носитель окончательного человеческого решения |
| **Допустимый следующий ход** | `accept`, `reject` или `reframe`; до него разделы 8–9 не разрешают выполнение работ |

Таким образом, этот документ сейчас предназначен прежде всего для решения по
`PAD-IWE-001`. Заполненный `PFAD-IWE-001` — зависимое решение только об устройстве
локального каркаса практики. Результат сравнения объясняет выбор варианта E, описание
архитектуры показывает выбранные структуры, а план перечисляет намеренную работу на
случай принятия решения.

### Реестр отдельных результатов и их маршрутов
...
| `PFAD-IWE-001` | Если выбран вариант E, как устроить издание локального каркаса практики: набор паттернов, зависимости, источники, публикацию и пересмотр? | `PrincipleFrameworkArchitectureDecision@Context` `PFAD-IWE-001` | `E.4.PFAD` | `PrincipleFrameworkArchitectureDecision@Context` | зависимое предложение; не утверждённый каркас | `amorales` в роли сопровождающего каркас, после или вместе с `PAD-IWE-001` |
| `AD-IWE-001` | Как показать выбранные структуры так, чтобы описание можно было использовать для проверки решения, не принимая его за архитектуру или решение? | `ArchitectureDescription@Context` `AD-IWE-001` над `ARCH-IWE-001` | `C.30.AD` | `ArchitectureDescription@Context` | действующее описание предложения | проверяющий `PAD-IWE-001`; после принятия — исполнители плана |
| `ECS-IWE-HARNESS-001` | Что означает «лучше» для точного издания одиночного запуска оболочки? | построение пространства характеристик оценки для `ExecutableHarnessEdition@PersonalIWE` | `A.19.ECS` | `EvaluationCharacteristicSpaceSpec` | черновая спецификация; не результат оценки | будущая оценка по `E.22`, затем цикл улучшения по `E.23` |
| `ECS-IWE-LOOP-001` | Что означает «лучше» для точного издания ограниченного исполнительного цикла? | построение пространства характеристик оценки для `BoundedLoopDefinitionEdition@PersonalIWE` | `A.19.ECS` | `EvaluationCharacteristicSpaceSpec` | черновая спецификация; не результат оценки | пилот цикла, затем `E.22` и `E.23` |
| `WORKPLAN-IWE-001` | Какие намеренные работы, зависимости, бюджеты и цели приёмки следуют **после** принятия решений? | `U.WorkPlan` `WORKPLAN-IWE-001` | `A.15.2` | `U.WorkPlan` из значений `PlanItem` | условный план; не выполненная и не разрешённая работа | владелец недельного плана и WP Gate после решения по `PAD-IWE-001` и `PFAD-IWE-001` |
...
наблюдения P0–P2
  ...
  → PAD-IWE-001: предложение владельцу принять архитектурный вариант и его потери
      → если принято: PFAD-IWE-001 и WORKPLAN-IWE-001 поступают на отдельные решения
      → если отклонено: возврат к P2S-IWE-001 или синтезу вариантов
      → если переопределено: меняются вопрос, набор кандидатов или основа сравнения
  → выполненная работа и наблюдения пилота
  → отдельные результаты оценки
  → сохранение решения либо возврат к PAD/PFAD по условиям пересмотра
```

Эта трасса не является планом работ. Она показывает передачу между результатами,
каждый из которых остаётся под своим прямым паттерном FPF.

## Резюме предлагаемого архитектурного решения
...
Этот раздел — повествовательная проекция `PAD-IWE-001` и `ARCH-IWE-001` для чтения
человеком. Он не является `ChoiceResult`, PFAD, планом работы, решением допуска или
свидетельством реализации. Статус всей публикации — предложение для человеческого
решения, а не утверждённая архитектура и не реализованная исполнительная среда.
...
Эта таблица — не терминологическое украшение. Она исправляет шесть текущих подмен:
...
### Объекты архитектурного утверждения, а не текущий объект решения

Текущий `EntityOfConcern` документа как предложения решения уже указан в разделе 0:
это `ArchitectureDecisionRelation@Project` `PAD-IWE-001`. Ниже перечислены объекты,
о структурах которых делается это архитектурное утверждение.
...
FPF использован не как общая инструкция модели, а как маршрутизатор текущих вопросов
к прямым `Solution`. Сначала устанавливается вид требуемого результата, затем его
`EntityOfConcern`, и только потом выбирается носитель публикации.

| Текущий вопрос | Карточка применения | Прямой `Solution` | Первый полезный результат | Куда он передаётся |
|---|---|---|---|---|
| Что за система строится и где её граница? | SYSTEM-IN-CONTEXT | A.1, C.30 | `ArchitectureQuestionCard@Project`, затем `ArchitectureOf@Context` | C.32.P2S и C.32.PAD |
| Как связать дефекты с предлагаемыми структурами? | ARCHITECTURE | C.32.P2S | `ProblemToStructureArchitecturingFlowCard@Project` | синтез кандидатов, C.11 и C.32.PAD |
| FMT, PACK, FPF, свой DPF или LPF с Control Center? | OPTION-COMPARISON | C.11 | `ChoiceResult` по зафиксированному `OptionSet` | `PAD-IWE-001`, не исполнение |
| Какое направление архитектуры принять для проекта? | ARCHITECTURE | C.32.PAD | `ArchitectureDecisionRelation@Project` | владельцу архитектуры для решения |
| Как устроить собственный локальный каркас принципов? | DPF-AUTHORING | E.4.PFAD | `PrincipleFrameworkArchitectureDecision@Context` | сопровождающему каркас после общего решения |
| Как показать предлагаемую архитектуру, не перепутав описание с ней самой? | DESCRIPTION-USE | C.30.AD | `ArchitectureDescription@Context` | проверяющему проектное решение |
| Что значит «улучшить» для оболочки и отдельно для цикла? | IMPROVEMENT | A.19.ECS | две `EvaluationCharacteristicSpaceSpec` | E.22 и E.23 после появления наблюдений |
| Как планировать внедрение после решения? | WORKING-DOCUMENTS | A.15.2 | условный `U.WorkPlan` и `PlanItem` | недельному планированию и проверке готовности A.15.5 |
| Как сделать допуски настоящими? | WORKING-DOCUMENTS / COSTLY-ACTION | A.21, A.10 | `GateDecision` и ссылки на свидетельства вместо напоминания | разрешённому или заблокированному переходу |
| Как планировать вызовы агента внутри одного запуска? | WORKING-DOCUMENTS | C.24 | рабочий договор и условный план вызовов с пределами и остановкой | исполнительной оболочке одного запуска |
| Как не загружать весь FPF? | DESCRIPTION-USE | E.11.PUA, E.17 | маршрутизация паттернов и закреплённый манифест контекста | текущему исполнителю |
| Как использовать внешний корпус знаний? | SOURCE-USE / CURRENTNESS | G.2, G.11, E.4.PFR | решение «принять — адаптировать — отклонить» с версией и условием пересмотра | паттерну или решению, использующему утверждение |
| Как не перепутать состояние человека с его устойчивой характеристикой? | SYSTEM-IN-CONTEXT / DESCRIPTION-USE | A.1, A.7, C.33, C.34 | ограниченный снимок состояния без скрытой психологической классификации | только объявленному текущему применению |
...
Первый отдельный результат для продолжения архитектурной работы —
[`P2S-IWE-G7-001`](WP-43-G7-PASS-WITHOUT-PUSH-P2S.md): трасса по `C.32.P2S` от
ложного `pass` к синтезу вариантов структуры исполнения, свидетельств, агрегирования
и публикации решения.
...
### Трасса от проблемного давления к структуре

Следующая запись отвечает только на вопрос о связности архитектурного рассуждения.
Она не выбирает вариант, не утверждает архитектуру и не разрешает работу.

```text
ProblemToStructureArchitecturingFlowCard@Project:
  flowId: P2S-IWE-001
  describedHolonRef: PersonalIWE@baseline-2026-07-16
  boundedContextRef: личная интеллектуальная работа amorales с Pi agent,
    Aethon, Codex и тремя рабочими репозиториями
  architectingHolonOrRoleRef: агент в роли архитектурного аналитика WP-43
  ...
  problemPressure:
    ...
    problemPressureSignalRefs:
      ...
      - P1: установленная среда перехватчиков не используется
      - P1: часть допусков является напоминаниями, а не внешними наблюдателями
      - P1: нет единой модели полномочий утверждений
      - P1: контекст выбирается документами, а не текущим применением
      - P1: три репозитория образуют неатомарное состояние
      - P2: показатели активности подменяют ценность
      - P2: протоколы расходятся с фактически развёрнутым поведением
    sourceUseRecordRefs:
      - FPF Core 2026-07
      - PACK-digital-platform@c924cee6
      - PACK-Personal@81f20ac7
      - Loop Engineering Working Note 2026, с ограничениями раздела 12
    currentStopOrReturnReason: общее архитектурное решение владельца ещё не принято

  architectureContent:
    candidateStructureKindRefs:
      - структура полномочий и источников
      - структура локального каркаса практики
      - управляющая структура экосистемы репозиториев
      - структура одиночного запуска оболочки
      - структура проверки и свидетельств
    ...
    architectureCharacteristicRefs:
      ...
      - наблюдаемость
      - восстановимость
      - целостность нескольких репозиториев
      - сопровождаемость
      - сохранение человеческого понимания и авторства
      - ограниченность стоимости и области последствий
    candidateSynthesisRef: варианты A–E раздела 4

  structuralInformation:
    unknownStructure:
      - фактический механизм согласованного изменения нескольких репозиториев
      - достаточный минимальный набор внешних допусков
      - граница, после которой ограниченный цикл даёт пользу, а не умножает ошибку
    selectedStructure: вариант E как аналитически предпочтительный кандидат
    expectedStructure: связка LPF → Control Center → одиночная оболочка →
      ограниченный цикл → свидетельства → пересмотр
    actualStructure: развитая интерактивная оболочка с текстовыми правилами,
      сценариями и частичными журналами, но без полного автономного цикла
    capturedInDescriptionsOrDecisions:
      - ARCH-IWE-001
      - AD-IWE-001
      - PAD-IWE-001
      - PFAD-IWE-001
    handedToMethodsOrWork: только условно через WORKPLAN-IWE-001 после решения
    latentOrHiddenStructure: реальные зависимости между правилами, сценариями,
      настройками Pi/Aethon и тремя репозиториями
    lostStructure: эксплуатационные свидетельства будущего пилота пока отсутствуют
    strongerStructureInspectionReturnCondition: несоответствие диаграммы фактическим
      перехватчикам, владельцам, потокам данных или состояниям восстановления

  decisionAndWorkDocking:
    candidateSetOrPaletteRef: OptionSet A–E
    architectureDecisionRef: PAD-IWE-001, proposedForHumanDecision
    methodDescriptionRefs:
      - текущие протоколы Открытие → Работа → Закрытие
      - предлагаемые паттерны LPF
    workPlanRefs: WORKPLAN-IWE-001, conditional
    readinessRefs: отсутствуют; проверяются отдельно по A.15.5
    performedWorkRefs: отсутствуют для предлагаемой архитектуры

  feedback:
    evalProgramRefs:
      - ECS-IWE-HARNESS-001
      - ECS-IWE-LOOP-001
    evalResultRefs: отсутствуют до пилота
    operationOrUseObservationRefs: аудит раздела 3
    governingPatternSpecificReturnOrRepair:
      c32PadOrAdaDecisionRepairOrSupersessionExit: условия пересмотра PAD-IWE-001
      e23ImprovementCycleRef: еженедельный цикл только после первой оценки
      c30DescriptionOrViewLossRepairRef: обновить AD-IWE-001 при расхождении с фактом

  governingPatternForNextClaim: C.11 для аналитического выбора, затем C.32.PAD
    для проектного решения владельца
```

Главный причинный мост трассы таков: P0 требует правдивого и ограниченного одиночного
запуска; P1 требует внешних наблюдателей, модели полномочий, сборки контекста и
согласования репозиториев; P2 требует отдельной оценки результата. Ограниченный цикл
следует только после этих структур, поскольку повторение ненадёжного запуска увеличит
расстояние жизни ошибки.

---

## 4. Результат сравнения вариантов (`ChoiceResult`)

- **Текущий вопрос:** какой вариант из уже сформированных A–E следует предпочесть и
может ли ещё одна ограниченная проверка изменить выбор?
- **`EntityOfConcern`:** зафиксированный выбор архитектурного аналитика WP-43 над
`OptionSet` A–E.
- **Применённый `Solution`:** `C.11`.
- **Точный вид результата:** `ChoiceResult` под явным `ChoiceRule`.
- **Получатель:** `PAD-IWE-001`; этот раздел не передаёт работу исполнителю.
...
### Основа сравнения, а не решение допуска

Шкала: 0–10. Э — эффективность, М — масштабируемость, О — отказоустойчивость,
Г — решение главной боли; по оси «Сложность» высокий балл означает простоту и
контролируемость реализации. Локальный порог этого сравнения: ни одна характеристика
не ниже 7.

| Вариант | Э | М | О | Г | Сложность | Сопровожд. | Безопасн. | Исход порогового правила |
|---|---:|---:|---:|---:|---:|---:|---:|---|
| A. Ответвление FMT | 5 | 6 | 6 | 4 | 6 | 4 | 7 | не проходит |
| B. Только PACK | 7 | 8 | 5 | 6 | 5 | 6 | 7 | не проходит |
| C. FPF напрямую | 6 | 8 | 5 | 6 | 3 | 5 | 7 | не проходит |
| D. Свой DPF сейчас | 6 | 7 | 6 | 5 | 3 | 3 | 7 | не проходит |
| **E. LPF + выбор источников + Control Center** | **9** | **8** | **8** | **9** | **7** | **8** | **8** | **проходит** |

Эта таблица является объявленной основой сравнения, но не `GateDecision` по A.21,
не свидетельством результата и не проектным архитектурным решением. Баллы являются
порядковыми оценками и не усредняются как физические измерения. Выбор следует из
порогового правила и видимых компромиссов, а не из одного итогового числа.

```text
ChoiceResult:
  choiceResultId: CHOICE-IWE-001
  DecisionSubject: агент в роли архитектурного аналитика WP-43
  DecisionSubjectGranularity: один проектный архитектурный разбор
  OptionSet:
    - A: продолжать развивать ответвление FMT
    - B: считать PACK готовым каркасом и единственным источником
    - C: использовать FPF напрямую как ежедневный регламент
    - D: сразу создать собственный DPF платформы
    - E: LPF + выборочное использование источников + Control Center
  EvaluativeMeasure:
    - каждая защищаемая характеристика не ниже 7 из 10
    - отсутствие стоп-фактора по главным характеристикам
    - сохранение человеческого суждения и авторства
    - прослеживаемость источников и зависимостей
    - возможность сначала доказать одиночный запуск
    - обратимость перехода и сопровождаемость
  BeliefState: статический аудит 2026-07-14, закреплённые источники и ограничения
    раздела 12; эксплуатационные данные предлагаемой архитектуры отсутствуют
  OutcomeModel: качественная оценка ожидаемых структурных последствий вариантов;
    не причинное доказательство эксплуатационной эффективности
  ProbeBudget: в рамках текущего архитектурного выбора допускается только проверка,
    способная изменить победивший вариант или отклонить весь OptionSet
  CostToProbe: полноценный пилот варианта E относится уже к проверке решения,
    а не к сравнению A–E до принятия направления
  ChoiceRule: выбрать вариант, который проходит все пороги, не имеет стоп-фактора,
    сохраняет границу человеческого решения и допускает проверяемый поэтапный переход
  result: choose now
  selectedOption: E
  whyNoFurtherProbeBeforeChoice: ни один названный дешёвый следующий поиск не меняет
    структурное преимущество E; неопределённость его эффективности проверяется пилотом
    и является условием сохранения или пересмотра решения
  nonAdmissibleUse: не считать этот результат принятием PAD-IWE-001, PFAD-IWE-001,
    разрешением работы или доказательством улучшения
  nextQuestion: должен ли владелец архитектуры принять PAD-IWE-001 с указанными
    потерями, последствиями и условиями пересмотра?
```

Вариант E должен быть проверен опытной эксплуатацией и повторной оценкой после первой
рабочей недели. Эта будущая проверка может сохранить, уточнить или открыть на пересмотр
архитектурное решение, но её отсутствие не превращает текущий `ChoiceResult` в
неопределённое «продолжать думать».
...
## 5. Решения, предложенные владельцу

### 5.1 Главный кандидат проектного архитектурного решения

- **Текущий вопрос:** следует ли принять вариант E как текущее направление архитектуры
личной IWE и признать перечисленные потери, последствия и условия пересмотра?
- **`EntityOfConcern`:** `ArchitectureDecisionRelation@Project` `PAD-IWE-001`.
- **Применённый `Solution`:** `C.32.PAD`.
- **Точный вид результата:** `ArchitectureDecisionRelation@Project`.
- **Состояние:** `proposedForHumanDecision`; выбранный вариант внутри записи является
предложением аналитика, а не уже совершённым решением владельца.
- **Получатель:** `amorales`, владелец архитектуры личной IWE.

```text
ArchitectureDecisionRelation@Project:
  decisionId: PAD-IWE-001
  decisionSubjectRef: amorales, владелец архитектуры личной IWE
  describedHolonRef: PersonalIWE@baseline-2026-07-16
  boundedContextRef: личная интеллектуальная работа amorales с Pi agent,
    Aethon, Codex и тремя рабочими репозиториями
  decisionQuestion: следует ли принять вариант E как текущее направление архитектуры
    личной IWE, не создавая пока собственный DPF и не включая автономный цикл до
    доказательства надёжности одиночного запуска?

  candidateBasisRefs:
    - P2S-IWE-001
    - варианты A–E раздела 4
    - аудит фактического состояния раздела 3
  comparisonOrSelectionRefs:
    - CHOICE-IWE-001: choose now E

  selectedArchitectureOptionRefs:
    - E: LPF + выборочное использование источников + Control Center +
      надёжный одиночный запуск + позднее ограниченный цикл

  selectedStructureEffects:
    - structureKindRef: структура полномочий и источников
      selectedStructureRef: FPF Core + первичные источники + решения об использовании PACK
      decisionEffect: заменить одну тотальную иерархию истины владельцами по видам утверждений
      governingPatternRef: G.2, G.11, E.4.PFR
    - structureKindRef: структура локальной практики
      selectedStructureRef: локальный каркас практики IWE
    ...
    - structureKindRef: структура исполнения одного запуска
      selectedStructureRef: оболочка с рабочим договором, внешними допусками,
        ограниченным набором изменений и свидетельствами
      decisionEffect: сначала сделать один запуск правдивым и восстанавливаемым
      governingPatternRef: A.21, C.24, A.15
    - structureKindRef: структура повторяющегося исполнения
      selectedStructureRef: необязательный ограниченный цикл с независимой проверкой
        и постоянным решением человека
      decisionEffect: допустить повторение только после прохождения порогов пилота
      governingPatternRef: E.22, E.23 и локальный LPF.BOUNDED-LOOP

  architectureCharacteristicTradeoffs:
    - architectureCharacteristicRef: надёжность и восстановимость
      expectedGain: ложный pass, частичный успех и потеря состояния становятся видимыми
      acceptedLoss: начальная стоимость внешних проверок, журналов и испытаний отказа
      guardrailRef: 0 скрытых частичных успехов и проверенный путь восстановления
    - architectureCharacteristicRef: человеческое понимание и авторство
      expectedGain: стратегия, архитектура, публикация и принятие риска остаются за человеком
      acceptedLoss: меньшая предельная скорость и невозможность полностью безлюдного цикла
      guardrailRef: непонятое изменение не принимается
    - architectureCharacteristicRef: сопровождаемость правил
      expectedGain: местные правила отделены от источников и средств исполнения
      acceptedLoss: нужно сопровождать PFAD, решения об использовании источников и
        соответствие описания фактически развёрнутому поведению
      guardrailRef: одно правило — один владелец — один исполняемый или человеческий выход
    - architectureCharacteristicRef: простота
      expectedGain: не создаётся новый репозиторий и собственный DPF до появления оснований
      acceptedLoss: в переходный период сохраняются карта старых имён и составной носитель
      guardrailRef: не более двух улучшений оболочки за один недельный цикл

  rationaleRefs:
    - CHOICE-IWE-001
    - проблемное давление и структурный мост P2S-IWE-001
    - дефекты P0/P1 и риск долгов цикла
    - требование сохранить человеческое суждение из PACK-Personal
  rejectedOptionRefs:
    - A: FMT остаётся образцом реализации, но не родительской архитектурой
    - B: PACK используется по отдельным утверждениям, но не как готовая система истинности
    - C: FPF маршрутизирует вопросы, но не становится ежедневным регламентом целиком
    - D: собственный DPF откладывается до появления переносимых доменных паттернов
  ...
  methodUseInstructions:
    - methodDescriptionRefOrPatternRef: рабочий договор и ограниченная фиксация области работы
      expectedStructureEffect: один запуск не захватывает чужое состояние
      responsibleRoleRef: исполнитель одиночного запуска
      workBoundaryRef: только принятые PlanItem после проверки готовности
    - methodDescriptionRefOrPatternRef: независимая проверка кандидата цикла
      expectedStructureEffect: создатель не удостоверяет собственный результат
      responsibleRoleRef: свежий проверяющий и человек, принимающий решение
      workBoundaryRef: только пилот после прохождения порогов одиночного запуска

  architectDeveloperSplit:
    architectOwnedStructureRefs:
      - границы LPF, Control Center, одиночного запуска и ограниченного цикла
      - матрица полномочий и постоянные точки человеческого решения
      - защищаемые характеристики и условия пересмотра
    developerOwnedRefinementRefs:
      - форматы журналов и снимков
      - реализация отдельных перехватчиков и тестов
      - удобство команд и представления Aethon внутри принятых границ
    sourceReturnCondition: вернуться к PAD-IWE-001, если уточнение меняет границу
      полномочий, выбранную структуру, принятую потерю или условие включения цикла

  publicationProjectionRef: этот составной файл является носителем предложения;
    отдельная ADR-проекция по C.32.ADR создаётся только после решения владельца
  evidenceOrAssuranceExitRefs:
    - будущие результаты испытаний P0 и восстановления
    - будущие оценки по ECS-IWE-HARNESS-001 и ECS-IWE-LOOP-001
  governanceExitRefs:
    - решение владельца accept/reject/reframe
    - WP Gate и A.15.5 только после accept
  reopenConditions:
    - пилот не достигает порогов надёжности, восстановимости или сохранения понимания
    - внешний наблюдатель не может реально остановить запрещённый переход
    - стоимость и ложные блокировки превышают установленные пределы
    - появляется более сильный источник, меняющий несущую структуру или принятую потерю
    - схема репозиториев или модель событий Pi/Aethon существенно меняется
  status: proposedForHumanDecision
```

Принятие `PAD-IWE-001` означает только принятие направления, границ и признанных
компромиссов. Оно не доказывает реализацию, не означает прохождение допусков и не
превращает условный план в выполненную работу.

### 5.2 Зависимый кандидат решения о локальном каркасе (`PFAD`)

- **Текущий вопрос:** если общее направление E принято, какое устройство должно иметь
первое издание локального каркаса практики IWE?
- **`EntityOfConcern`:** `PrincipleFrameworkArchitectureDecision@Context` с
идентификатором `PFAD-IWE-001`.
- **Применённый `Solution`:** `E.4.PFAD`.
- **Точный вид результата:** `PrincipleFrameworkArchitectureDecision@Context`.
- **Состояние:** зависимое предложение, а не утверждённый каркас и не ADR.
- **Получатель:** `amorales` в роли сопровождающего локальный каркас, после или вместе
с решением по `PAD-IWE-001`.

`PFAD-IWE-001` не отвечает за всю личную IWE. Он решает только специфические для
каркаса вопросы: издание, набор паттернов, связи, источники, имена, зависимости,
носители доступа, оценку качества и условия обновления.
...
PrincipleFrameworkArchitectureDecision@Context:
  ...
  namingDecisionRefs:
    - «локальный каркас практики IWE» — рабочее русское имя семейства
    - сокращение LPF сохраняется только как устоявшееся обозначение FPF
    - перед первой публикацией требуется отдельная проверка долговечного имени по F.18
  ...
  qualityEvaluationRefs:
    - ECS-IWE-HARNESS-001 как черновая спецификация, не результат оценки
    - ECS-IWE-LOOP-001 как черновая спецификация, не результат оценки
    - E.21 для качества каждого опубликованного паттерна
    - E.23 только после появления исходной оценки точного издания

  admissionReviewRefs:
    - отсутствуют: допуск каркаса, внешняя готовность и посадка пока не заявляются
  ...
  rationaleRefs:
    - PAD-IWE-001, только если он принят владельцем
    - CHOICE-IWE-001
    - P2S-IWE-001
    - матрица владельцев утверждений из раздела 3
  ...
  publicationCarrierRefs:
    - до принятия: только этот носитель предложения
    - после принятия: отдельная проекция решения и издание локального каркаса;
      сами носители не становятся каркасом
...
## 6. Описание предлагаемой архитектуры

- **Текущий вопрос архитектурного утверждения:** какие выбранные структуры составляют
предлагаемую архитектуру личной IWE в указанном контексте?
- **`EntityOfConcern`:** `ArchitectureOf@Context` `ARCH-IWE-001`.
- **Применённый `Solution`:** `C.30`.
- **Точный вид результата:** `ArchitectureOf@Context`.
- **Получатель:** `PAD-IWE-001` как основание для проектного решения.

- **Текущий вопрос описания:** как сделать это утверждение проверяемым через несколько
структурных представлений, не приняв диаграмму или текст за саму архитектуру?
- **`EntityOfConcern`:** описание `AD-IWE-001` архитектурного утверждения
`ARCH-IWE-001`.
- **Применённый `Solution`:** `C.30.AD`.
- **Точный вид результата:** `ArchitectureDescription@Context`.
- **Получатель:** проверяющий `PAD-IWE-001`; после принятия — владелец условного плана.

```text
ArchitectureOf@Context:
  architectureClaimId: ARCH-IWE-001
  describedHolonRef: PersonalIWE@baseline-2026-07-16
  boundedContextRef: личная интеллектуальная работа amorales с Pi agent,
    Aethon, Codex и тремя рабочими репозиториями
  structureRefs:
    - структура полномочий человека, источников, каркаса и средств исполнения
    - структура локального каркаса практики IWE
    - структура Control Center для согласования экосистемы
    - структура одиночного запуска управляющей оболочки
    - структура рабочих продуктов, событий, свидетельств и восстановления
    - необязательная структура ограниченного исполнительного цикла
  structureKindRefs:
    - control structure
    - method and work structure
    - role and authority structure
    - information and evidence structure
    - module and interface structure
    - transformation-flow structure
  architectureConcernCue: controlOrRateMismatch + dataCustodyOrStateResidence +
    runtimeFailureMode + crossScopeResidual + descriptionViewLoss
  governingArchitectureConcernRefs:
    - P2S-IWE-001
    - PAD-IWE-001
    - PFAD-IWE-001
  admissibleUse: проверить полноту выбранных структур и последствия предлагаемого
    решения; после принятия — уточнять принадлежащие разработчику структуры
  nonAdmissibleUse: не считать утверждение принятым решением, описанием выполненной
    работы, свидетельством надёжности или разрешением на запуск цикла

ArchitectureDescription@Context:
  architectureDescriptionId: AD-IWE-001
  architectureClaimRef: ARCH-IWE-001
  descriptionContext:
    EntityOfConcernRef: ARCH-IWE-001
    BoundedContextRef: личная IWE amorales
    ViewpointRef: владелец архитектуры, проверяющий решения и будущий исполнитель
  describedHolonRef: PersonalIWE@baseline-2026-07-16
  selectedStructureRefs:
    - источники и границы полномочий
    - локальный каркас практики
    - Control Center
    - одиночный запуск оболочки
    - ограниченный цикл
    - состояние, продукты, свидетельства и улучшение
  structureKindRefs:
    - authority, control, module, interface, information, evidence,
      work and transformation-flow structures
  architectureStructuralViewRefs:
    - общая структурная схема ниже
    - представление источников и полномочий, раздел 6.1
    - представление Control Center и сборки контекста, разделы 6.2–6.3
    - представление одиночного запуска и допусков, разделы 6.4–6.8
    - представление ограниченного цикла и проверки, разделы 6.9–6.11
    - представление размещения по репозиториям, раздел 6.12
  correspondenceRefs:
    - структуры схемы ↔ selectedStructureEffects PAD-IWE-001
    - LPF на схеме ↔ selectedPatternSetRefs PFAD-IWE-001
    - исполнительные части ↔ условные PlanItem WORKPLAN-IWE-001
  sourceEpistemeRefs:
    - FPF Core 2026-07
    - закреплённые версии PACK и Loop Engineering с ограничениями раздела 12
    - локальный аудит раздела 3
  sourceReturnCondition: вернуться к исходным решениям и фактическому состоянию,
    если представление скрывает владельца, границу, принятый компромисс,
    восстановление или точку решения человека
  freshnessCueRefs:
    - изменение модели событий Pi/Aethon
    - изменение состава или ролей репозиториев
    - изменение принятого PAD или PFAD
  specificationUseBoundary: до принятия PAD описание служит только разбору;
    после принятия задаёт архитектурные границы, но не подменяет готовность A.15.5
  publicationUseBoundary: этот файл — составной носитель; диаграмма не является
    архитектурой, решением, допуском или свидетельством
  admissibleUse: проверка решения, восстановление связей и подготовка уточнений
  nonAdmissibleUse: автоматическое разрешение работ, слияния, публикации или запуска
```
...
## 7. Спецификации оценки: что значит «лучше»

Здесь нет результата оценки и нет утверждения, что предлагаемая архитектура лучше
исходной в эксплуатации. Раздел содержит две разные черновые спецификации пространств
характеристик. Они разделены, потому что издание одиночного запуска и издание
повторяющегося цикла — объекты разных видов с разными режимами отказа.

### 7.1 Спецификация оценки одиночного запуска

- **Текущий вопрос:** какие характеристики, основания, пороги и защищаемые компромиссы
позволят оценить точное издание оболочки одиночного запуска?
- **`EntityOfConcern`:** построение пространства характеристик оценки для
`ExecutableHarnessEdition@PersonalIWE`.
- **Применённый `Solution`:** `A.19.ECS`.
- **Точный вид результата:** `EvaluationCharacteristicSpaceSpec`.
- **Получатель:** будущая постановка оценки по `E.22`; после получения значений —
цикл улучшения по `E.23`.

```text
EvaluationCharacteristicSpaceSpec:
  specId: ECS-IWE-HARNESS-001
  EvaluatedObjectKindRef: ExecutableHarnessEdition@PersonalIWE
  ObjectVersionUnderImprovementRef: baseline-2026-07-16
  DeclaredUseScope: оценка пригодности одного точного издания оболочки для
    совместной интеллектуальной работы и предложение не более двух улучшений
  WorkingReaderScope: владелец IWE, исполнитель оболочки и независимый проверяющий
  QualificationWindow: одна рабочая неделя или минимум 10 содержательных сеансов
  DiscriminatingCaseSet:
    admissible: точное издание, где обязательные переходы имеют исполняемые решения,
      ссылки на свидетельства и проверенный путь восстановления
    belowFloor: исходное издание с ложным pass, неограниченной фиксацией или
      обещанием протокола без фактического исполнения
    outsideDeclaredObjectKind: компетентность, продуктивность или психологическая
      характеристика человека
  ObjectKindFitRule: оценивается одно издание оболочки; человек, отдельный рабочий
    продукт, план и LoopDefinition возвращаются к своим оценкам
  CharacteristicSlotSet: координаты раздела 7.2
  ScaleBindingSet: пороговые, долевые, временные и доказательно обоснованные
    порядковые значения; скрытое усреднение запрещено
  PolarityAndPreferredMovement: уменьшать ложный успех, время восстановления,
    лишний контекст и трение при сохранении понимания, приватности и переносимости
  FloorAndExceptionalMeaningSet: пол задают все обязательные пороги 0.1;
    высокий результат не снимает обязательные границы человеческого решения
  EvaluationEvidenceBasisRule: точное издание настроек и кода, журнал решений,
    различия, проверки, записи результата, испытания отказа и выборка объяснений владельца
  EvidenceAndMissingnessRule: отсутствие нужного основания понижает соответствующую
    координату или делает значение неизвестным; оно не превращается в pass
  ResultRowShape: coordinate, value, rationale, evidenceRefs, lowerOrReopenCondition
  AdjacentValueRationaleRule: для порядкового значения объяснить, почему соседнее
    ниже недостаточно, а соседнее выше ещё не подтверждено
  CalibrationPointSet: требуется дополнить после первых 10 сеансов;
    до этого сравниваются пороги и наблюдаемые дефекты, а не общий балл
  ProtectedTradeoffSet:
    - человеческое суждение и авторство
    - простота, приватность и переносимость
    - практика мышления письмом
    - время и стоимость контекста
  StatusValueSet: draftNeedsCalibration | readyForUse | belowFloor | viableForUse
  StopOrReopenCondition: остановить текущий цикл улучшения при достижении всех
    обязательных порогов; открыть снова при ложном pass, невосстановимом состоянии,
    росте ложных блокировок или утрате понимания
  E22QuestionFrameUse: floorEvaluation + candidateImprovementProposalEvaluation
  E23StartCondition: точное издание, исходные значения и проверяемые основания
    за одно квалификационное окно
  status: draftNeedsCalibration
```

### 7.2 Координаты и пороги одиночного запуска
...
### 7.3 Спецификация и дополнительные координаты ограниченного цикла
...
- **Текущий вопрос:** какие характеристики и пороги показывают, что точное издание
ограниченного цикла уменьшает время жизни ошибки, не разрушая человеческое суждение?
- **`EntityOfConcern`:** построение пространства характеристик оценки для
`BoundedLoopDefinitionEdition@PersonalIWE`.
- **Применённый `Solution`:** `A.19.ECS`.
- **Точный вид результата:** `EvaluationCharacteristicSpaceSpec`.
- **Получатель:** пилот точного издания цикла, затем отдельные результаты оценки по
`E.22` и решение об улучшении по `E.23`.

```text
EvaluationCharacteristicSpaceSpec:
  specId: ECS-IWE-LOOP-001
  EvaluatedObjectKindRef: BoundedLoopDefinitionEdition@PersonalIWE
  ObjectVersionUnderImprovementRef: точное издание будущего пилота
  DeclaredUseScope: решить, допускается ли повторение технического обслуживания
    IWE без команды человека на каждый запуск
  WorkingReaderScope: владелец IWE, создатель, независимый проверяющий
  QualificationWindow: минимум 10 содержательных запусков одного издания
  DiscriminatingCaseSet:
    admissible: один кандидат, изоляция, независимая проверка, сохраняемое состояние,
      жёсткие пределы и обязательное решение человека
    belowFloor: цикл, который переносит обязательный отказ, забывает состояние,
      позволяет самопроверку или превышает пределы
    outsideDeclaredObjectKind: одиночный интерактивный запуск, недельный план,
      стратегия или качество человека
  ObjectKindFitRule: оценивается одно неизменное издание LoopDefinition;
    изменения определения начинают новое квалификационное окно
  CharacteristicSlotSet: дополнительные координаты ниже
  ScaleBindingSet: доли, счётчики переходов, пределы затрат и доказательно
    обоснованные порядковые решения; общий средний балл запрещён
  PolarityAndPreferredMovement: уменьшать ложные кандидаты, долг проверки,
    расстояние жизни ошибки и стоимость без ослабления остановки и решения человека
  FloorAndExceptionalMeaningSet: все обязательные пороги пилота достигнуты;
    отсутствие reject или abstain после 10 кандидатов требует калибровки
  EvaluationEvidenceBasisRule: неизменное издание LoopDefinition, журнал переходов,
    исходы детерминированных проверок, решения свежего проверяющего и человека,
    испытания остановки и превышения пределов
  EvidenceAndMissingnessRule: пропущенный обязательный исход означает belowFloor,
    а не неизвестное благополучие
  ResultRowShape: coordinate, value, rationale, evidenceRefs, failureDistance,
    lowerOrReopenCondition
  CalibrationPointSet: формируется на первых 10 запусках; до завершения пилота
    нет утверждения о пригодности запуска без наблюдения
  ProtectedTradeoffSet:
    - человеческое суждение, понимание и авторство
    - независимость проверяющего
    - простота и ограниченность стоимости
    - неизменность полномочий внутри одного издания
  StatusValueSet: draftNeedsPilot | belowFloor | viableForObservedUse |
    viableForBoundedUnattendedUse
  StopOrReopenCondition: остановить цикл при обязательном отказе, превышении предела,
    потере состояния, ожидании человека или несоответствии понимания;
    открыть архитектурное решение при повторяемом структурном дефекте
  E23StartCondition: завершённый пилот одного точного издания с исходными значениями
  status: draftNeedsPilot
```
...
### 7.4 Реестр четырёх долгов цикла
...
### 7.5 Будущий цикл улучшения управляющей оболочки
...
## 8. Условный план намеренной работы (`U.WorkPlan`)

- **Текущий вопрос:** какие намеренные работы, зависимости, бюджеты и цели приёмки
следуют, если владелец примет архитектурное направление и зависимое решение о
локальном каркасе?
- **`EntityOfConcern`:** план намеренной работы `WORKPLAN-IWE-001`.
- **Применённый `Solution`:** `A.15.2`.
- **Точный вид результата:** `U.WorkPlan`, состоящий из значений `PlanItem`.
- **Состояние:** `conditionalDraft`; это не выполненная работа, не решение готовности и
не разрешение начать работу.
- **Получатель:** владелец недельного плана и WP Gate; готовность каждого принятого
пункта проверяется отдельно по `A.15.5`.

```text
U.WorkPlan:
  workPlanId: WORKPLAN-IWE-001
  intendedWorkFamilyRef: переход от baseline-2026-07-16 к принятой архитектуре
    личной IWE 0.1
  boundedContextRef: WP-43 и последующие явно принятые рабочие пакеты
  horizon: пять зависимых фаз и отдельное квалификационное окно минимум 10 запусков
  cadence: фазовый переход; еженедельная оценка начинается только после исходных значений
  intendedRoleValues:
    - amorales: владелец архитектуры, приоритетов, принятия риска и слияния
    - агент-исполнитель: ограниченное изменение в принятой области работы
    - независимый проверяющий: попытка опровергнуть результат без контекста создателя
    - Control Center: снимок, маршрутизация, согласование и восстановление без
      полномочий содержательного решения
  plannedConstraints:
    - PAD-IWE-001 и PFAD-IWE-001 не считаются принятыми по факту публикации файла
    - одна фаза не разрешает следующую без достижения своей цели приёмки
    - автоматические слияние, отправка, изменение стратегии и самоизменение цикла запрещены
    - ранее изменённые файлы не включаются в область текущей работы без явного основания
  plannedResourceBudget: ориентир 22–36 часов плюс 10 пилотных запусков;
    каждая фаза требует отдельного допуска в фактический WeekPlan
  plannedBaseline: содержимое и фактическое состояние baseline-2026-07-16;
    перед выполнением каждого PlanItem состояние измеряется заново
  expectedVarianceRelation: выполненная U.Work ссылается на PlanItem и отдельно
    записывает отклонение времени, области, метода, приёмки и назначенной роли

  PlanItems:
    - planItemId: PI-0-TRUTHFULNESS
      targetMethodRef: исправление критических дефектов одиночного запуска
      plannedWindow: первая принятая работа, до архитектурной перестройки
      plannedResourceBudget: 1–2 часа
      dependencies: допускается независимо от принятия PAD как ограниченное
        исправление P0, но только после обычного WP Gate
      acceptanceTarget: ни одно закрытие не фиксирует посторонние пути и не
        публикует pass без свидетельств
    - planItemId: PI-1-LPF-SOURCES
      targetMethodRef: решение и первое издание локального каркаса практики
      plannedWindow: после решения владельца
      plannedResourceBudget: 4–6 часов
      dependencies: PAD-IWE-001 accepted; PFAD-IWE-001 accepted или уточнён
      acceptanceTarget: для типичного сеанса восстановимы паттерн, вид результата,
        граница делегирования и способ исполнения
    - planItemId: PI-2-CONTROL-CENTER
      targetMethodRef: минимальный Control Center и восстановимое изменение репозиториев
      plannedWindow: после определения полномочий и источников
      plannedResourceBudget: 4–6 часов
      dependencies: PI-1-LPF-SOURCES; актуальный снимок репозиториев
      acceptanceTarget: частичный успех имеет явное состояние, владельца и
        проверенный путь восстановления
    - planItemId: PI-3-SINGLE-HARNESS
      targetMethodRef: пилот исполняемой оболочки одиночного запуска
      plannedWindow: после минимального Control Center
      plannedResourceBudget: 5–8 часов и минимум 10 сеансов
      dependencies: PI-0-TRUTHFULNESS; PI-2-CONTROL-CENTER;
        ECS-IWE-HARNESS-001 readyForUse
      acceptanceTarget: релевантный контекст и решения обязательных переходов
        реально управляют изменением и оставляют проверяемые свидетельства
    - planItemId: PI-4-BOUNDED-LOOP
      targetMethodRef: пилот одного ограниченного цикла технического обслуживания
      plannedWindow: только после достижения порогов одиночного запуска
      plannedResourceBudget: 4–6 часов и 10 запусков одного издания
      dependencies: PI-3-SINGLE-HARNESS accepted; ECS-IWE-LOOP-001 readyForUse;
        испытания аварийной остановки, abstain и ожидания человека
      acceptanceTarget: обязательный отказ не переживает переход; пределы работают;
        слияние и переопределение остаются решением человека
    - planItemId: PI-5-SIMPLIFY-IMPROVE
      targetMethodRef: удаление дублей и ограниченное еженедельное улучшение
      plannedWindow: после первого оценённого пилота, затем еженедельно
      plannedResourceBudget: 4–8 часов на упрощение; далее не более двух изменений в неделю
      dependencies: результаты оценки точных изданий, а не активность или впечатление
      acceptanceTarget: каждое обязательное правило имеет одного владельца,
        один исполняемый или человеческий выход и путь к свидетельствам

  status: conditionalDraft
```

Ни одна отметка ниже не означает, что работа выполнена или даже принята в текущий
недельный план. При исполнении создаётся отдельная запись `U.Work` и связь исполнения
или отклонения с соответствующим `PlanItem`.
...
## 9. Условный срез `WORKPLAN-IWE-001` для W29 (~4 ч)
...
Этот срез является уточнением значений `PlanItem`, а не вторым архитектурным решением,
не записью выполненной работы и не автоматическим изменением текущего WeekPlan.
...
| 0.5 ч | принять либо вернуть на переработку PAD и PFAD, матрицу полномочий и границу делегирования | явное состояние двух решений и получатель следующего хода |
...
- [`FPF/FPF-Spec.md`](FPF/FPF-Spec.md), издание 2026-07: карточки практического
  применения, A.1, A.10, A.15.2, A.19.ECS, A.21, C.11, C.24, C.30, C.30.AD,
  C.32.P2S, C.32.PAD, C.33, C.34, E.4, E.4.PFAD, E.4.DPF, E.4.DPF.DA,
  E.4.PFR, E.22, E.23, G.2 и G.11;
- [`AGENTS.md`](AGENTS.md);
...
6. Предложение `PAD-IWE-001` основано на статическом аудите и истории рабочих
   продуктов. Нужны пилотные наблюдения и повторная оценка, прежде чем объявлять
   оболочку улучшенной.
...
Текущий результат для решения теперь однозначен: `PAD-IWE-001` вида
`ArchitectureDecisionRelation@Project`. Его получатель — `amorales`; допустимый
следующий ход — принять, отклонить или переопределить предложение. `CHOICE-IWE-001`
объясняет, почему аналитически выбран вариант E, но не совершает это человеческое
решение. `PFAD-IWE-001` решает более узкий вопрос о каркасе, а
`WORKPLAN-IWE-001` остаётся условным намерением до принятия решений и проверки
готовности.
...
Если `PAD-IWE-001` принят, первый приоритет плана — не новый PACK, не собственный DPF
и не расписание, а **правдивое закрытие, изменения только в области текущей работы,
